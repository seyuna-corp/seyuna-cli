...
pub async fn spinner_start(text: &str) -> RunningSpinner {
    let s = Spinner::new(text).start();
    sleep(Duration::from_millis(500)).await; // Shorter delay for snappier UX
    s
}
...
pub async fn spinner_progress(s: &RunningSpinner, text: &str) {
    s.text(text).update();
    sleep(Duration::from_millis(500)).await; // Shorter delay for responsiveness
}
...
pub async fn spinner_done(s: &RunningSpinner, text: &str) {
